        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 [column-fill:_auto]">
          {GALLERY_PHOTOS.map((photo, idx) => (
            <motion.div
              ...
              className="break-inside-avoid mb-6 relative group rounded-2xl overflow-hidden shadow-sm border border-[#e5e5e7] bg-[#eeeeef]"
            >
              <div className={`${photo.aspect} w-full overflow-hidden relative`}>
                {/* Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-white/95 via-white/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col justify-end p-6" />
                ...
                {/* Text inside overlay */}
                <div className="absolute bottom-6 left-6 right-6 z-20 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                  <span className="text-[8px] uppercase tracking-wider font-bold text-[#76767b] block mb-1">Eagle Space</span>
                  <span className="text-sm font-bold text-[#1d1d1f] block">{photo.title}</span>
                </div>
              </div>
            </motion.div>
          ))}
